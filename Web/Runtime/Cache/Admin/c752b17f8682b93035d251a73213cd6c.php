...
<div class=" page-content clearfix">
    <div id="products_style">
        <div class="search_style"> 
            ...
            <form action="<?php echo U('Goods/productsList');?>" method="post">
        ...
        <div class="border clearfix">
            <span class="l_f">
                <a href="<?php echo U('Goods/pictureAdd');?>" title="添加商品" class="btn btn-warning Order_form"><i class="icon-plus"></i>添加商品</a>
                <!-- <a href="/Test/Public/Admin/javascript:ovid()" class="btn btn-danger"><i class="icon-trash"></i>批量删除</a> -->
                <a href="<?php echo U('paramAdd');?>" class="btn btn-info">添加商品参数模板</a>
            </span>
            ...
            <table class="table table-striped table-bordered table-hover" id="sample-table">
            	...
            	<tbody>
                    <?php if(is_array($list)): foreach($list as $k=>$v): ?><tr>
                            ...
                            <td width="100px"><?php echo ($v['promotion_price']); ?></td>         
                            <td width="180px"><?php echo ($v['addtime']); ?></td>
                            <td class="td-status"><span class="label label-success radius" onclick="amend(this, <?php echo ($v['id']); ?>)" id="sp_status" style="cursor:pointer"><?php echo ($v['status']); ?></span></td>
                            <td class="td-manage">
                                <a title="编辑" onclick="" href="<?php echo U('Goods/amendPicture',['id'=>$v['id']]);?>"  class="btn btn-xs btn-info" ><i class="icon-edit bigger-120"></i></a> 
...
<script>
    ...
    function amend(obj, id) {
        $.ajax({
            url:"<?php echo U('Goods/amendGoods');?>",
            data:'id='+id,
            type:'post',
            success:function(res) {
                if (res == 1) {
