...
<body>
    ...
    <div class="type_title" style="height: 40px; width: 1172px;">修改商品</div>
    	<form action="<?php echo U('Goods/edit');?>" method="post" class="form form-horizontal" id="form-article-add" enctype="multipart/form-data">
        <!--隐藏域传商品id -->
        <input type="hidden" name="id" value="<?php echo ($goods['id']); ?>"/>
            <div class="clearfix cl">
                ...
                <div class="formControls col-10" id="grandpa">
                    <select class="form-control" name="tid">
                        <option>--请选择--</option>
                        <?php if(is_array($type)): foreach($type as $key=>$v): ?><option <?php echo ($goods['tid']==$v['id']?'selected':''); ?> value="<?php echo ($v['id']); ?>"><?php echo ($v['name']); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
            </div>
            <div class="clearfix cl">
                <label class="form-label col-2"><span class="c-red">*</span>商品名称：</label>
                <div class="formControls col-10"><input type="text" style="height: 30px; width:500px;"class="input-text" value="<?php echo ($goods['name']); ?>" placeholder="" id="" name="name"></div>
            </div>
    		<div class=" clearfix cl">
                <label class="form-label col-2">简略标题：</label>
    	        <div class="formControls col-10"><input type="text" class="input-text" value="<?php echo ($goods['subtitle']); ?>" placeholder="" id="" name="subtitle" style="height: 30px; width:500px;"></div>
    		</div>
            <div class=" clearfix cl">
                <label class="form-label col-2">原价格：</label>
                <div class="formControls col-10"><input type="number" class="input-text" value="<?php echo ($goods['price']); ?>" placeholder="" id="" name="price" style="height: 30px; width:500px;"></div>
            </div>
            <div class=" clearfix cl">
                <label class="form-label col-2">促销价格：</label>
                <div class="formControls col-10"><input type="number" class="input-text" value="<?php echo ($goods['promotion_price']); ?>" placeholder="" id="" name="promotion_price" style="height: 30px; width:500px;"></div>
            </div>
            <div class=" clearfix cl">
            ...
            <div class=" clearfix cl">
                ...
                <div class="formControls col-10"><input type="text" class="input-text" value="<?php echo ($goods['desc']); ?>" placeholder="请输入关键字，用逗号隔开" id="" name="desc" style="height: 30px; width:500px;"></div>
            </div>
            
            <div class="">
                <a href="#">　　<span class="glyphicon glyphicon-plus-sign" id="add-property">添加属性</span></a>
            </div>
            <?php if(is_array($property)): foreach($property as $key=>$v): ?><div class="clearfix cl" id="property">
                <div class="Add_p_s">
                    <label class="form-label col-2">口味：</label>
                    <div class="formControls col-2">
                        <input name="taste[]" value="<?php echo ($v['taste']); ?>" />
                    </div>
                </div>
                <div class="Add_p_s">
                    <label class="form-label col-2">价格：</label>
                    <div class="formControls col-2">
                        <input type="number" name="now_price[]" value="<?php echo ($v['now_price']); ?>"/>
                    </div>
                </div>
                <div class="Add_p_s">
                    <label class="form-label col-2">库存：</label>
                    <div class="formControls col-2">
                        <input type="number" name="repertory[]" value="<?php echo ($v['repertory']); ?>" />
                    </div>
                </div>
